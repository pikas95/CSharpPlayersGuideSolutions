internal class GameProgram
{
    ...
    private List<Contractor> _contractorStore;
    ...
    private void ExpeditionMenu()
    {
        while (true)
        {
            ...
            int input = _input.AskForIntInRange("Which expedition you wish to conquer? ", 0, _expeditions.Count);
            ...
            while (_expeditions[input - 1] == null) // displayed as index + 1 in console
            {
                ...
                input = _input.AskForIntInRange("No such option. Again: ", 0, _expeditions.Count);
            }
            
            
            if (_player.Contractors.Count != 0)
            {
    ...
    private void RunExpedition(Expedition expedition)
    {
        ...
        List<Contractor> unpaidContractors = _player.PayContractorsAfterExpedition();

        _renderer.DisplayExpeditionResult(expedition, _player, unpaidContractors);
    ...
    private void HealMenu()
    {
        ...
        {
            for (int i = 0; i < _player.Contractors.Count && !_player.AllContractorsFullHP(); i++)
            {
    ...
    private void ContractorStore()
    {
        while (true)
        {
            ...
            _renderer.DisplayContractorStoreActions(_player.Contractors.Count != 0);
            _renderer.DisplayReturnOption();

            int action = _input.AskForIntInRange("What do you want to do? ", 0, _player.Contractors.Count != 0 ? 2 : 1);
        ...
        void DismissContractor()
        {
            ...
            int input = _input.AskForIntInRange("Which contractor you wish to dismiss? ", 1, _player.Contractors.Count) - 1;
            ...
            _contractorStore.Add(dismissed);
        }
    }
    ...
    {
        List<Contractor> unpaidContractors = _player.PayContractorsAfterExpedition();
        _renderer.DisplayUnpaidContractors(unpaidContractors);
        _contractorStore = ContractorStoreGenerator.Generate(_contractorStoreSize);
    }
